...
export default function RiderDashboard() {
	...
	const fetchBookings = async () => {
		try {
			...
			const response = await fetch('http://localhost:5000/api/v1/rider/accepted-bookings', {
				headers: {
	...
	const riderBookings = useMemo(
		() =>
			bookings.map((booking, index) => {
				...
				return {
					id: booking.reference_number || booking.id || `booking-${index + 1}`,
					dbId: booking.id,
		...
		<div className="min-h-screen bg-white px-4 py-6 sm:py-10">
			<div className="mx-auto w-full max-w-2xl md:max-w-5xl lg:max-w-6xl">
				...
				<div className="grid gap-4 md:grid-cols-2">
					{loading && <p className="col-span-full text-sm text-[#3878c2]">Searching for bookings...</p>}
					{error && <p className="col-span-full text-sm text-[#e55353]">{error}</p>}
					{!loading && !error && riderBookings.length === 0 && (
						<p className="col-span-full text-sm text-[#374151]">No accepted bookings available at the moment.</p>
					)}
